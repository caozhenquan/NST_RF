...
void LT_WriteBUF_ble(unsigned char reg, unsigned char *pBuf, unsigned char len)
{
	unsigned char i;

	RF_SS=0;
	SPI_SendByte(reg); 	
	SPI_SendByte(len+4);	//Lenth
	SPI_SendByte(0xd6);	
	SPI_SendByte(0xbe);	
	SPI_SendByte(0x89);	
	SPI_SendByte(0x8e);	
...
unsigned char LT_ReadBUF_ble(unsigned char reg, unsigned char *pBuf,unsigned char lenth)
{
	unsigned char i,len;
